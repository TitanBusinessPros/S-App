...
- **3D Maps** (`Map3D.tsx`, `/app/map3d`): a tilted, rotatable 3D view of the user's location, using OpenFreeMap's Liberty style rendered via plain MapLibre GL directly (no Leaflet involved this time) — the same style/tile service the removed "street" basemap used, but a different integration path deliberately chosen to avoid the Leaflet-binding bug that broke that one, and because the Leaflet binding can't do pitch/bearing at all, which a 3D view requires. "3D" isn't a distinct OpenFreeMap product — it's the same flat vector style viewed at a pitch (60°) and bearing (55.2°), matching OpenFreeMap's own demo values, just recentered on the user instead of a fixed London default.

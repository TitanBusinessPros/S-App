...
- ~~OpenStreetMap/Nominatim usage risk at real scale~~ — **resolved (2026-09-07)**. A 2026-09-07 cost review (prompted by "what would this cost at 10,000 daily users") found the Water & Terrain Map's basemap tiles (`tile.openstreetmap.org`) and the reverse-geocoding call in `getLocationName` (`nominatim.openstreetmap.org`) both ran on OSM Foundation's free public infrastructure, whose usage policy explicitly warns commercial/heavy use may be blocked without notice. The tile half moved to OpenFreeMap (see "Water & Terrain Map" above); the geocoding half (`getLocationName`, also used by Plants, Wildlife & Wood for its location label) now calls the U.S. Census Bureau's Geocoder instead of Nominatim — verified directly against its Terms of Service first (no commercial-use restriction, matching the same bar applied to OpenFreeMap and USGS). Coverage is US/Puerto Rico/Island Areas only, which is fine for this app's audience. Both OSM Foundation dependencies are now gone from the app entirely.
